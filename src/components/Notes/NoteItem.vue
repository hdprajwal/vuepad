...
<template>
    ...
    <RouterLink :to="`/note/${note.id}`" v-else
        class="p-4 border rounded-lg bg-card w-[250px] h-[300px] relative overflow-hidden flex flex-col group">
        <div class="flex-1 relative overflow-hidden">
            ...
            <div
                ...
                <button @click.stop.prevent="deleteNote(note.id)"
                    class="absolute bottom-1 right-1 hidden pointer-events-auto group-hover:inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring hover:bg-destructive hover:text-destructive-foreground h-8 w-8 text-destructive">
                    <Trash class="w-4 h-4" />
                </button>
            </div>
        </div>
